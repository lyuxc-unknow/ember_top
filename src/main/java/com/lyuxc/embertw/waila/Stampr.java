...
public class Stampr {
    public static class ember implements IWailaDataProvider {
        ...
        public List<String> getWailaBody(ItemStack itemStack, List<String> tips, IWailaDataAccessor wailaDataAccessor, IWailaConfigHandler wailaConfigHandler) {
            ...
            double ember = ((TileEntityStamper) Objects.requireNonNull(wailaDataAccessor.getTileEntity())).capability.getEmber();
            double maxember = ((TileEntityStamper) Objects.requireNonNull(wailaDataAccessor.getTileEntity())).capability.getEmberCapacity();
            tips.add(I18n.format("random.power")+":"+ember+"/"+maxember);

            return tips;
        }
        @Override
        public NBTTagCompound getNBTData(EntityPlayerMP entityPlayerMP, TileEntity entity, NBTTagCompound compound, World world, BlockPos blockPos) { return null; }
    }
}
